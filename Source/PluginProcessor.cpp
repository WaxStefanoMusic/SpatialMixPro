...
{
    addParameter(paramX     = new AudioParameterFloat ({"x",  1}, "Pos X",    -8.0f,  8.0f,  0.0f));
    addParameter(paramY     = new AudioParameterFloat ({"y",  1}, "Pos Y",     0.0f,  6.0f,  1.5f));
    addParameter(paramZ     = new AudioParameterFloat ({"z",  1}, "Pos Z",    -8.0f,  8.0f, -3.0f));
    addParameter(paramGain  = new AudioParameterFloat ({"g",  1}, "Volume",    0.0f,  2.0f,  1.0f));
    ...
    addParameter(paramX2    = new AudioParameterFloat ({"x2", 1}, "Pos X2", -8.0f,  8.0f,  2.0f));
...
void SpatialMixProProcessor::prepareToPlay(double sampleRate, int samplesPerBlock)
{
    ...
    dBufL1.assign(dBufSize, 0.0f); dBufR1.assign(dBufSize, 0.0f);
    dBufL2.assign(dBufSize, 0.0f); dBufR2.assign(dBufSize, 0.0f);
    dWritePos1 = dWritePos2 = 0;

    dsp::ProcessSpec spec { sampleRate, (uint32)samplesPerBlock, 1 };
    elevFL1.prepare(spec); elevFL1.reset();
    elevFR1.prepare(spec); elevFR1.reset();
    elevFL2.prepare(spec); elevFL2.reset();
    elevFR2.prepare(spec); elevFR2.reset();
    rearFL1.prepare(spec); rearFL1.reset();
    rearFR1.prepare(spec); rearFR1.reset();
    rearFL2.prepare(spec); rearFL2.reset();
    rearFR2.prepare(spec); rearFR2.reset();
    subFilterL.prepare(spec); subFilterL.reset();
    subFilterR.prepare(spec); subFilterR.reset();

    auto subC  = dsp::IIR::Coefficients<float>::makeLowPass(sampleRate, 120.0f, 0.707f);
    subFilterL.coefficients = subC; subFilterR.coefficients = subC;
    auto rearC = dsp::IIR::Coefficients<float>::makeLowPass(sampleRate, 3500.0f, 0.707f);
    ...
    smRear1.reset(sampleRate, 0.1); smRear1.setCurrentAndTargetValue(0.0f);
    smRear2.reset(sampleRate, 0.1); smRear2.setCurrentAndTargetValue(0.0f);
}
...
{
    float gainLin = 1.0f + jmax(0.0f, (elevDeg - 10.0f) / 80.0f) * 2.0f;
    auto c = dsp::IIR::Coefficients<float>::makeHighShelf(
                 currentSampleRate, 8000.0f, 0.707f, gainLin);
    fl.coefficients = c;
    fr.coefficients = c;
}
...
void SpatialMixProProcessor::processBlock(AudioBuffer<float>& buffer, MidiBuffer&)
{
    ...
    {
        float cx = smX.getTargetValue();
        float cy = smY.getTargetValue();
        float cz = smZ.getTargetValue();
        ...
        if (isDual)
        {
            float cx2 = smX2.getTargetValue();
            float cy2 = smY2.getTargetValue();
            float cz2 = smZ2.getTargetValue();
            float hd2 = std::sqrt(cx2 * cx2 + cz2 * cz2);
            float el2 = std::atan2(cy2, jmax(hd2, 0.001f)) * (180.0f / MathConstants<float>::pi);
...

    {
        ...
        smRear1.setTargetValue(jmax(0.0f, -std::cos(az1)));
        if (isDual)
        {
            float az2 = std::atan2(paramX2->get(), -(paramZ2->get()));
            smRear2.setTargetValue(jmax(0.0f, -std::cos(az2)));
        }
    }
    ...
    for (int i = 0; i < numSamples; ++i)
    {
        ...
        const float panR1  = std::sqrt(jmax(0.0f, 0.5f * (1.0f + sinAz1)));
        ...
        if (mode == 0)
        {
            const float itdSec  = (0.215f / 343.0f) * std::sin(az1);
            ...
            if (az1 > 0.0f) rL1 = (dWritePos1 - itdSamp + dBufSize) % dBufSize;
            ...
            outL1 = eL1 * (1.0f - rear1) + rlL1 * rear1;
            outR1 = eR1 * (1.0f - rear1) + rlR1 * rear1;
        ...
        if (isDual)
        {
            ...
            const float panR2  = std::sqrt(jmax(0.0f, 0.5f * (1.0f + sinAz2)));
            ...
            if (mode == 0)
            {
                const float itdSec2 = (0.215f / 343.0f) * std::sin(az2);
                ...
                if (az2 > 0.0f) rL2 = (dWritePos2 - itdS2 + dBufSize) % dBufSize;
                ...
                outL2 = eL2 * (1.0f - rear2) + rlL2 * rear2;
                outR2 = eR2 * (1.0f - rear2) + rlR2 * rear2;
...
void SpatialMixProProcessor::setStateInformation(const void* data, int sz)
{
    ...
    *paramGain  = (float)st.getProperty("g",    1.0f);
    paramMode->setValueNotifyingHost((int)st.getProperty("mode", 0) / 2.0f);
    paramMono ->setValueNotifyingHost((bool)st.getProperty("mono",  false) ? 1.0f : 0.0f);
    paramPhase->setValueNotifyingHost((bool)st.getProperty("phase", false) ? 1.0f : 0.0f);
    paramDual ->setValueNotifyingHost((bool)st.getProperty("dual",  false) ? 1.0f : 0.0f);
    *paramX2    = (float)st.getProperty("x2",   2.0f);
